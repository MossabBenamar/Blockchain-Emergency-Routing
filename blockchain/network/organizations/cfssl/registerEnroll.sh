...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.emergency.net/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.emergency.net/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.emergency.net/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.emergency.net/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.emergency.net/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.emergency.net

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.emergency.net-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.emergency.net-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.emergency.net

    mkdir -p organizations/ordererOrganizations/emergency.net/ca
    mkdir -p organizations/ordererOrganizations/emergency.net/msp/cacerts
    mkdir -p organizations/ordererOrganizations/emergency.net/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/emergency.net/orderers
    mkdir -p organizations/ordererOrganizations/emergency.net/tlsca

    CERT_DIR=organizations/ordererOrganizations/emergency.net

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.emergency.net-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.emergency.net-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/emergency.net/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/emergency.net/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.emergency.net-cert.pem"
